...
const BookDetail = ({ navigation, route }) => {
  ...
  const [processing, setProcessing] = useState(false); // <--- 서재 등록/삭제 처리 상태
  ...
  const loadBookData = async () => {
    ...
    try {
      ...
      if (bookData) {
        ...
        setBook(null); // 표시할 데이터가 전혀 없는 경우
      }
  ...
  const loadQuestions = async () => {
    ...
    try {
      ...
      if (data.isSuccess && data.result && data.result.questions) {
        ...
        // API에서 정렬을 지원하더라도, 클라이언트에서 한 번 더 적용 (selectedSort가 useEffect 종속성에 포함되어 있으므로)
  ...
  // 질문 등록 함수: 이미 authenticatedFetch 사용
  const handleAddQuestion = async (questionData) => {
    ...
    try {
      ...
      const response = await authenticatedFetch(`${API_BASE_URL}/api/books/${book.bookId}/questions`, {
        method: 'POST',
        body: JSON.stringify({
  ...
  // 질문 삭제 함수: 이미 authenticatedFetch 사용
    ...
    Alert.alert(
      ...
      [
        { text: '취소', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              console.log('질문 삭제 요청:', `${API_BASE_URL}/api/questions/${questionId}`);

              const response = await authenticatedFetch(`${API_BASE_URL}/api/questions/${questionId}`, {
                method: 'DELETE',
              });
              ...
              if (data.isSuccess || response.status === 204) { // 204 No Content도 성공으로 처리
                await loadQuestions();
  ...
  // 질문 클릭 시 질문 상세 화면으로 이동하는 함수 (변경 없음)
        ...
        <View style={styles.questionSection}>
          <View style={styles.bookSection}>
            ...
            <View style={styles.bookInfo}>
              ...
              {/* 내 서재 등록/삭제 버튼에 handleAddOrDeleteBook 함수 연결 */}
              {book.isInLibrary ? (
                <TouchableOpacity
